...
openai.api_key = "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"
...
def gerar_srt(segments, srt_path="transcricao.srt"):
...
def analisar_transcricao(transcricao):
    ...
    prompt = f"""
    Abaixo está a transcrição de um vídeo. Analise e divida os trechos em duas categorias: pontos relevantes e pontos irrelevantes. Os pontos relevantes são aqueles que devem ser mantidos no vídeo final, e os irrelevantes podem ser removidos. Não precisa ser tão criterioso, mas mantenha logica nas analises. vou criar um unico video com sua sugestao relevante, juntando-as para ter um video logico.

    {transcricao}

    Sugira apenas os trechos que contêm os momentos mais importantes, em minutos e segundos.
    Formato de resposta (OBS.: Não alterar este modelo de saída!):
    Pontos relevantes:
    - De 00:01 a 03:00: Discussão sobre investimento
    - De 06:00 a 10:00: Conclusão financeira
...
def juntar_videos(cortes):
    """Junta os cortes relevantes em um único vídeo final mantendo a qualidade original."""
    if not cortes:
        print("Nenhum corte para juntar.")
        return

    # Cria um arquivo temporário para listar os arquivos a serem concatenados
    file_list_path = "temp/file_list.txt"
    with open(file_list_path, "w") as f:
        for corte in cortes:
            f.write(f"file '{os.path.abspath(corte)}'\n")

    # Comando FFmpeg para concatenação sincronizada e fps definido como 60
    try:
        command = (
            "ffmpeg -y -loglevel error -f concat -safe 0 "
            f"-i {file_list_path} "
            "-vf 'fps=60' -c:v libx264 -crf 18 -preset veryfast "
            "-c:a aac -b:a 192k -movflags +faststart video_final.mp4"
        )
...
def extrair_cortes(video_path, sugestoes):
    ...
    for inicio, fim in trechos_relevantes:
        try:
            ...
            # Ignora cortes menores que 10 segundos
...
def processar_video_para_cortes(video_path):
    ...
    juntar_videos(cortes)

    print("Processo completo!")

video = "teste.mp4"
processar_video_para_cortes(video)
